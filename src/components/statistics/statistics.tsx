...
const Statistics = () => {
  ...
  const generateCompetencies = (): Competency[] => {
    ...
    for (let competency of initialConfig.competencies) {
      ...
      const groups = groupIds.map((id) => {
        const index = initialConfig.groups.findIndex(
          (group) => group.id === id
        );
      ...
      for (let group of groups) {
        const allCourses: Course[] = getCoursesFromGroups([group]).map((c) => {
          const index = curriculumCourses.findIndex(
        ...
        const constraint = initialConfig.constraints.xOutOfYConstraints.find(
          (c) => c.group === group.id
        );
        if (constraint !== undefined) {
          allCoursesEcts += constraint.maxEcts;
        } else {
          allCoursesEcts += allCourses
            .map((c) => c.ects)
            .reduce((x1, x2) => x1 + x2, 0);
        }
        finishedCoursesEcts += allCourses
  ...
  const generateData = (): ChartData<"doughnut", number[], string> => {
    ...
    for (let competency of generateCompetencies()) {
      const doneValue = Math.min(
        competency.finishedCoursesEcts,
        competency.allCoursesEcts
      );
      const notDoneValue = Math.max(
        competency.allCoursesEcts - competency.finishedCoursesEcts,
        0
      );
      ...
      if (notDoneValue !== 0) {
        ...
        backgroundColor.push(
          competency.color.alpha(0.5).string() || "rgba(0, 0, 0, 0.5)"
        );
        borderColor.push(competency.color.string() || "rgba(0, 0, 0, 0.5)");
        hoverBackgroundColor.push(
          competency.color.alpha(0.5).darken(0.4).string()
        );
        hoverBorderColor.push(competency.color.darken(0.4).string());
      }
  ...
  return (
    <div className="App">
      <Container sx={{ padding: "2rem", height: "100%" }} maxWidth={"xl"}>
        <div
          style={{ position: "relative", height: "65vh", paddingTop: "5rem" }}
        >
